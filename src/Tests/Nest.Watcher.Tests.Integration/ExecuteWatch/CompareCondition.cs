...
{
	[TestFixture]
	public class CompareCondition : IntegrationTest
	{
			...
			var executeWatch = this.Client.ExecuteWatch(new ExecuteWatchRequest
				{
					...
					Watch = new PutWatchRequest
					{
						...
						Condition = new ConditionContainer(new EqualCondition("ctx.payload.foo", "bar")),
		...
		protected virtual void Assert(IExecuteWatchResponse response)
		{
			...
			response.WatchRecord.Result.Condition.Type.Should().Be(ConditionType.Compare);
